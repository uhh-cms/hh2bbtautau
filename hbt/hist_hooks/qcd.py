...
def add_hooks(config: od.Config) -> None:
    ...
    def fake_factor_incl(task, hists):
        ...
        for group_name in complete_groups:
            ...
            # SS positive
            channels[group_name]["ss_pos_iso_mc"] = ss_pos_iso_mc
            channels[group_name]["ss_pos_iso_data"] = ss_pos_iso_data
            channels[group_name]["ss_pos_noniso_mc"] = ss_pos_noniso_mc
            channels[group_name]["ss_pos_noniso_data"] = ss_pos_noniso_data
            # SS negative
            channels[group_name]["ss_neg_iso_mc"] = ss_neg_iso_mc
            channels[group_name]["ss_neg_iso_data"] = ss_neg_iso_data
            channels[group_name]["ss_neg_noniso_mc"] = ss_neg_noniso_mc
            channels[group_name]["ss_neg_noniso_data"] = ss_neg_noniso_data
        ...
        for group_name in complete_groups:
            for k in ["incl"]:  # INDICATE WHICH CATEGORY TO CALCULATE THE FACTOR FOR ! e.g. "incl", "2j" ...
                ...
                if group_name == f"etau__{k}":
                    ...
                    # SS positive
                    ss_pos_iso_mc = channels[f"etau__{k}"]["ss_pos_iso_mc"] + channels[f"mutau__{k}"]["ss_pos_iso_mc"] + channels[f"tautau__{k}"]["ss_pos_iso_mc"]  # noqa: E501
                    ss_pos_iso_data = channels[f"etau__{k}"]["ss_pos_iso_data"] + channels[f"mutau__{k}"]["ss_pos_iso_data"] + channels[f"tautau__{k}"]["ss_pos_iso_data"]  # noqa: E501
                    ss_pos_noniso_mc = channels[f"etau__{k}"]["ss_pos_noniso_mc"] + channels[f"mutau__{k}"]["ss_pos_noniso_mc"] + channels[f"tautau__{k}"]["ss_pos_noniso_mc"]  # noqa: E501
                    ss_pos_noniso_data = channels[f"etau__{k}"]["ss_pos_noniso_data"] + channels[f"mutau__{k}"]["ss_pos_noniso_data"] + channels[f"tautau__{k}"]["ss_pos_noniso_data"]  # noqa: E501
                    # SS negative
                    ss_neg_iso_mc = channels[f"etau__{k}"]["ss_neg_iso_mc"] + channels[f"mutau__{k}"]["ss_neg_iso_mc"] + channels[f"tautau__{k}"]["ss_neg_iso_mc"]  # noqa: E501
                    ss_neg_iso_data = channels[f"etau__{k}"]["ss_neg_iso_data"] + channels[f"mutau__{k}"]["ss_neg_iso_data"] + channels[f"tautau__{k}"]["ss_neg_iso_data"]  # noqa: E501
                    ss_neg_noniso_mc = channels[f"etau__{k}"]["ss_neg_noniso_mc"] + channels[f"mutau__{k}"]["ss_neg_noniso_mc"] + channels[f"tautau__{k}"]["ss_neg_noniso_mc"]  # noqa: E501
                    ss_neg_noniso_data = channels[f"etau__{k}"]["ss_neg_noniso_data"] + channels[f"mutau__{k}"]["ss_neg_noniso_data"] + channels[f"tautau__{k}"]["ss_neg_noniso_data"]  # noqa: E501
                    ...
                    # choose signal region. Note: must be one of the qcd groups, e.g. group.os_iso)
                    signal_region = group.os_iso
                    print("using ss_pos_noniso_qcd as denominator!")
                    # --------------------------------------------------------------------------------------

                    # calculate the pt-independent fake factor
                    int_numerator = integrate_num(num_region, axis=1)
                    int_denominator = integrate_num(den_region, axis=1)
                    fake_factor_int = (int_numerator / int_denominator)[0, None]

                    # calculate the pt-dependent fake factor
                    fake_factor = (num_region / den_region)[:, None]
                    fake_factor_values = np.squeeze(np.nan_to_num(fake_factor()), axis=0)
                    fake_factor_variances = fake_factor(sn.UP, sn.ALL, unc=True)**2
