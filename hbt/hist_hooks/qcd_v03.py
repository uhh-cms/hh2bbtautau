...
def add_hooks(config: od.Config) -> None:
    ...
    def qcd_v03(
        ...
        if method_I:
            ...
            shape_region, numerator = control_regions[0], control_regions[1]
        ...
        signal_region = task.config_inst.get_category(signal_region_name)
    ...
    def qcd_all_configs(
        ...
        # initialize dictionaries for later use
        qcd_groups_per_config: dict[od.Config, dict[str, dict[str, od.Category]]] = defaultdict(DotDict)
        group_names_per_config: dict[od.Config, list[str]] = defaultdict(list)
        hists_per_config = dict[od.Config, list] = defaultdict(list)

        # build temporary config dictionary for testing
        my_cfg_dic = {}
        my_cfg_dic[config] = hists
        # loop over available configs
        # for config, hists in hists.items():
        for cfg, hists in my_cfg_dic.items():
            # get the qcd process
            qcd_proc = cfg.get_process("qcd", default=None)
            if (not qcd_proc or not hists):
                logger.warning("No hists or no qcd process found. Exiting hook...")
                return hists
            ...
            for cat_id in category_ids:
                cat_inst = cfg.get_category(cat_id)
                if cat_inst.has_tag({os_tag, noniso_tag}, mode=all):
                    qcd_groups_per_config[cfg][cat_inst.x.qcd_group].os_noniso = cat_inst
                elif cat_inst.has_tag({ss_tag, iso_tag}, mode=all):
                    qcd_groups_per_config[cfg][cat_inst.x.qcd_group].ss_iso = cat_inst
                elif cat_inst.has_tag({ss_tag, noniso_tag}, mode=all):
                    qcd_groups_per_config[cfg][cat_inst.x.qcd_group].ss_noniso = cat_inst

            # get category names without qcd regions, e.g. "etau__incl"
            group_names_per_config[config] = [name for name, cats in qcd_groups_per_config[cfg].items()]

            # nothing to do if there are no complete groups
            if not group_names_per_config[cfg]:
                return hists
            ...
            hists_per_config[cfg] = [mc_hist, data_hist]
        ...
        if all_channels is False:
            signal_region = f"{channel}__" + signal_region
        for cat_index in range(cat_axis.size):
            if cat_axis.value(cat_index) == signal_region.id:
                qcd_hist.view().value[cat_index, ...] = qcd_estimation_values
                qcd_hist.view().variance[cat_index, ...] = qcd_estimation_variances
    ...
    # add all hooks
    config.x.hist_hooks.qcd_v03 = qcd_v03
    config.x.hist_hooks.qcd_all_configs = qcd_all_configs
