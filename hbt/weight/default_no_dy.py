...
def default_no_dy(self: WeightProducer, events: ak.Array, **kwargs) -> ak.Array:
...
@default_no_dy.init
def default_no_dy_init(self: WeightProducer) -> None:
    ...
    if dataset_inst:
        if not dataset_inst.has_tag("dy"):
...
normalization_inclusive = default_no_dy.derive(
    "normalization_inclusive",
    cls_dict={"drop_weights": {"normalization_weight"}},
)


normalization_only = default_no_dy.derive(
    "normalization_only",
    cls_dict={"keep_weights": {"normalization_weight"}},
)

normalization_inclusive_only = default_no_dy.derive(
