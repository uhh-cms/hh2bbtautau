...
"""
Generic interface for loading and evaluating Torch models in a separate process.
...
class TorchEvaluator:
    ...
    def __init__(self) -> None:
        ...
        self._models: dict[str, TorchEvaluator.Model] = {}
    ...
    def add_model(self, name: str, path: str | pathlib.Path) -> None:
        ...
        # add it
        self._models[name] = TorchEvaluator.Model(name=name, path=path)
...
def _torch_evaluate(
    ...
    *,
    ...
    _print("importing torch ...")
    import numpy as np
    import torch  # type: ignore[import-not-found,import-untyped]
    _print("done")

    # helper to cast numpy arrays to torch tensors
    def cast_from_numpy(obj: Any) -> Any:
        if isinstance(obj, (list, tuple)):
            return type(obj)(cast_from_numpy(o) for o in obj)
        if isinstance(obj, dict):
            return type(obj)((k, cast_from_numpy(v)) for k, v in obj.items())
    ...
    class Model:
        ...
        def evaluate(self, *args, **kwargs) -> np.ndarray:
            ...
            with torch.no_grad():
                args = cast_from_numpy(args)
                kwargs = cast_from_numpy(kwargs)
