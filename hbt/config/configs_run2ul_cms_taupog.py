...
import order as od  # type: ignore
from scinum import Number  # type: ignore

from columnflow.util import DotDict, dev_sandbox  # type: ignore
from columnflow.config_util import (
    get_root_processes_from_campaign, add_shift_aliases, get_shifts_from_sources,
    verify_config_processes,
)  # type: ignore
from columnflow.columnar_util import ColumnCollection  # type: ignore
...
def add_config(
    ...
    if cfg.campaign.x("custom", {}).get("name") == "cms_taupog":
        def get_dataset_lfns(
            ...
            dataset_top_name = dataset_id.split("_")[0]
            year_prefix = "Run2" if year < 2019 else "Run3"
            lfn_base = law.wlcg.WLCGDirectoryTarget(
                f"/{cfg.campaign.x.custom['tag']}/{year_prefix}_{year}/{dataset_top_name}/",
                fs=f"{cfg.campaign.x.custom['wlcg_fs']}",
            )
        ...
        def get_dataset_lfns_fs(dataset_inst: od.Dataset) -> list[str]:
            ...
            fs.append(f"{cfg.campaign.x.custom['wlcg_fs']}")
            return fs
