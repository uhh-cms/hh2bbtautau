...
class stiched_process_ids_1d(stitched_process_ids):
    """
    Process identifier for subprocesses spanned by a single observable, such
    as DY LO, which have (e.g.) "*_1j" as well as "*_pt100to200" subprocesses.
    """
    ...
    def setup_func(self, task: law.Task, **kwargs) -> None:
        # define stitching ranges for the DY datasets covered by this producer's dy_inclusive_dataset
        stitching_ranges: dict[VarRange, ] = {}
        for proc in self.leaf_processes:
            var = proc.x(self.var_aux)
            stitching_ranges.setdefault(var, [])
    ...
    def key_func(
        self,
        var: int | float | np.ndarray,
    ) -> tuple[int, ] | tuple[np.ndarray, ]:
        # potentially convert single values into arrays
        single = False
        if isinstance(var, int):
            var = np.array([var], dtype=(np.int32))
            single = True

        # map into bins (index 0 means no binning)
        var_bins = np.zeros(len(var), dtype=np.int32)
        for var_bin, (var_min, var_max) in enumerate(self.sorted_stitching_ranges, 1):
            var_mask = (var_min <= var) & (var < var_max)
            var_bins[var_mask] = var_bin
