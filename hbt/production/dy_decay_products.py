...
def dy_decay_products(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    ...
    """
    Creates a new ragged column "higgs_family" that stores the tops and their decay products. The structure will
    be as follows:
    ...
    # Distinguish 3 cases: Z0 is child of particle -1, Z0 is grandchild of particle -1, Z0 does not exist in GenPart
    # list:
    # case 1:
    ...
    # case 2:
    case2_z0 = first_particles.distinctChildren.distinctChildren[first_particles.distinctChildren.
        distinctChildren.pdgId == 23]
    ...
    case_3_leps = attach_coffea_behavior_fn(case_3_leps, collections={"GenPart": {"type_name": "GenParticle",
    ...
    tau_children = ak.mask(leptons, tau_mask).distinctChildrenDeep[ak.mask(leptons, tau_mask).distinctChildrenDeep.
        hasFlags("isFirstCopy")]
    ...
    case_3_lep_mask = ak.num(ak.drop_none(case_3_leps), axis=1) > 0
    leptons = attach_coffea_behavior_fn(leptons, collections={"GenPart": {"type_name": "GenParticle", "check_attr":
        "metric_table", "skip_fields": "*Idx*G"}})
    ...
    # z0_leps_attached = attach_coffea_behavior_fn(z0_leps, collections = {"GenPart":{"type_name": "GenParticle",
    # "check_attr": "metric_table", "skip_fields": "*Idx*G"}})
    # inv_mass_z0_leps = z0_leps_attached.sum(axis=1).absolute()
    # events = set_ak_column(events, "inv_mass_z0_leps", inv_mass_z0_leps)

    # quarks_attached = attach_coffea_behavior_fn(mother_quarks, collections = {"GenPart":{"type_name": "GenParticle",
    # "check_attr": "metric_table", "skip_fields": "*Idx*G"}})
