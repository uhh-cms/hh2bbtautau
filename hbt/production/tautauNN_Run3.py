...
@producer(
    uses={
        ...
        "Jet.btagDeepFlavB", "Jet.btagPNetB",
        "Jet.btagDeepFlavCvB" , "Jet.btagDeepFlavCvL",
        "Jet.btagPNetCvB", "Jet.btagPNetCvL", "Jet.btagPNetQvG",
        "Jet.hhbtag",
        "MET.pt", "MET.phi",
...
def tautauNN_Run3(
    ...
    **kwargs,
    ...
    # input_features defined at https://github.com/uhh-cms/tautauNN/blob/f1ca194/evaluation/interface.py#L67
    ...
    # input_features defined at: https://github.com/uhh-cms/tautauNN/blob/main/tautaunn/config.py#L512
    contionus_input_features = [
        # MET kinematics, DeepMETResponseTune and covariance matrix
        "met_px", "met_py", "met_cov00", "met_cov01", "met_cov11",
        # DAU kinematics
        "dau1_e", "dau1_px", "dau1_py", "dau1_pz",
        "dau2_e", "dau2_px", "dau2_py", "dau2_pz",
        # BJET kinematics
        "bjet1_e", "bjet1_px", "bjet1_py", "bjet1_pz", "bjet2_e", "bjet2_px", "bjet2_py", "bjet2_pz",
        # BTAG scores, pnet and hhbtag
    ...
    categorical_input_features = [
        # channel_id, 0=muon_tau, 1=electron_tau, 2=tau_tau
        "pair_type",
        # dau decay mode, -1 for lepton, 0 for 1-prong, 1 for 3-prong, 10 for 1-prong+pi0, 11 for 3-prong+pi0
        "dau1_dm", "dau2_dm", "dau1_charge", "dau2_charge",
    ...
    # has_bjet_pair
    # defined in https://github.com/uhh-cms/tautauNN/blob/5c5371bc852f2a5c0a40ce0ecd9ffaa0c360e565/tautaunn/config.py#L632
    ...
    f.year = ak.full_like(f.met_px, self.year, dtype=np.int32)
    ...
    # these masks are in the interface but not used right now
    ...
    masked_network_inputs = {
        "cat_input": tf.cast(tf.boolean_mask(cat_inputs, selection_mask), np.int32),
        "cont_input": tf.cast(tf.boolean_mask(cont_inputs, selection_mask), np.float32),
    }
...
@tautauNN_Run3.requires
def tautauNN_Run3_requires(self: Producer, reqs: dict) -> None:
    """
...
@tautauNN_Run3.setup
def tautauNN_Run3_setup(self: Producer, reqs: dict, inputs: dict, reader_targets: InsertableDict) -> None:
    """
...
def apply_tautauNN_Run3_selection_cut(events: ak.Array, **kwargs) -> ak.Array:
    pre_selection_cut: str | list[str] | dict[str, list[str]] = [
        "nbjetscand > 1",
        "nleps == 0",
        "isOS == 1",
        "dau2_deepTauVsJet >= 5",
        (
            "((pair_type == 0) & (dau1_iso < 0.15) & (isLeptrigger == 1)) | "
            "((pair_type == 1) & (dau1_eleMVAiso == 1) & (isLeptrigger == 1)) | "
            "((pair_type == 2) & (dau1_deepTauVsJet >= 5))"
        ),
    ],
