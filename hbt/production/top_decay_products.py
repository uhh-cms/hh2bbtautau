...
def top_decay_products(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    ...
    events = self[attach_coffea_behavior](events, collections = {"top_family":{"type_name": "GenParticle", "check_attr": "metric_table", "skip_fields": "*Idx*G"}}, **kwargs)
...
#only mother flags (last copy always) -> even worse
# @producer(
#     uses={"GenPart.*", attach_coffea_behavior},
#     produces={"top_family.*", "reco_top_mass", "top_mass"},
# )
...
#     tops_children = tops_children[tops_children.hasFlags(*mother_gen_flags)]
...
#     bottoms = ak.flatten(bottoms, axis = 2)
#     w_bosons = tops_children[abs(tops_children.pdgId) == 24]
#     w_children = w_bosons.distinctChildren
#     w_children = w_children[w_children.hasFlags(*mother_gen_flags)]
#     w_bosons = ak.flatten(w_bosons, axis = 2)
#     qq = w_children[w_children.pdgId > 0]
#     qq = qq[qq.pdgId < 5]
#     qq = ak.flatten(qq, axis = 3)
...
#     #empty
#     # other_top_children = tops_children[abs(tops_children.pdgId) != 5]
#     # other_top_children = other_top_children[abs(other_top_children.pdgId) != 24]
#     # from IPython import embed; embed(header="debugger")


#     #TODO: add additional w products
...
#     tops_inv_mass = top_family[:,0,0].absolute()
